...
public:
    ...
    MyVector& operator=(MyVector& other) {
        if (this != &other) {
            delete[] _MyVector;
            _MyVector = new T[other.capacity_];
            for (size_t i = 0; i < other.size_; ++i) _MyVector[i] = other.arr_[i];
            _size = other.size_;
            _capacity = other.capacity_;
        }
        return *this;
    }
...
MyVector<T>::MyVector(const MyVector &copyMyVector) : _size(copyMyVector._size),_capacity(copyMyVector._capacity)
{
    _MyVector = new T[_capacity];
    for(int i = 0; i < _size;i++){
        _MyVector[i] = copyMyVector._MyVector[i];
    }
}
...
MyVector<T>::MyVector(MyVector &&moveMyVector) : _size(moveMyVector._size),_capacity(moveMyVector._capacity)
{
    _MyVector = new T[_capacity];
    for(int i = 0; i < _size; i++){
        _MyVector[i] = moveMyVector._MyVector[i];
    }
}
