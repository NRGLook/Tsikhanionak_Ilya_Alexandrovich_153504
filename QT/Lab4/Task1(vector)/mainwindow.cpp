...
void MainWindow::updateMyVectorTable(){
...
/*void MainWindow::on_getAnswer_clicked()
{
    ...
    --n;
    MyVector<int> answer = sequence[n / 2].second;
...
void MainWindow::Normalize(MyVector<int>& c) {
...
void MainWindow::AddZero(MyVector<int>& c, int x) {
    c.resize(x + 1);
    c[x] = 0;
}

MyVector<int> MainWindow::AddNum(const MyVector<int>& a, const MyVector<int>& b, int k) {
...
int MainWindow::GetBit(const MyVector<int>& v, int b) {
    return v[b] & 1;
}

MyVector<int> MainWindow::Multiply(const MyVector<int>& a, int x, int k) {
...
void MainWindow::GenerateSequence(MyVector<int> n, const MyVector<MyVector<int>>& p, int k, int i, MyVector<pair<int, MyVector<int>> >& v) {
