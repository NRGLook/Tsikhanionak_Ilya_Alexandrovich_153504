...
public:
    ...
    struct Node
    {
        ...
        {
            parent = prev;
            KeyValue = new std::pair<const Key, Value>(k, newData);
            //KeyValue.first = k;
            //KeyValue.second = newData;
            left = right = 0;
            height = 1;
        }
    ...
    {
        Node* y = node->right; //pivot - pottential new node
        node->right = y->left;
    ...
    /* WITHOUT parent in Node, old realisation
    ...
    */
    ...
    Node* insert(Key k, Value newData, Node* prev, Node* p)
    {
        ...
        key_compare comparator1; // compare 2 values
        if (comparator1(k, p->KeyValue->first))//check c and firstValue
        {
            bool checkDuplicate = false;
            if (p->left == nullptr)
            {
                if (p->KeyValue->first == k)    // duplicate
                {
                    checkDuplicate = true;
                }
                prev = p;
    ...
    Node* getNodeByKey(Node* root, Key value) {
        while (root) {
            ...
            //try to find element
            if (comparator(value, root->KeyValue->first))
            {
                root = root->left;
                continue;
            }
            else if (comparator(root->KeyValue->first, value))
            {
                root = root->right;
                continue;
            }
    ...
    Node* remove(Key k, Node* p)
    {
        ...
        std::greater<Key> comparator2;//change order sorting - for easy sorting
        if (comparator1(k, p->KeyValue->first))
            p->left = remove(k, p->left);
        else if (comparator2(k, p->KeyValue->first))
    ...
    void insertToList(std::list<Node*>& list, Node* node)
    {
        ...
        {
            auto it = list.begin();
            while (it != list.end() && key_compare().operator()((*it)->KeyValue->first, node->KeyValue->first))
                ++it;
            list.insert(it, node);
        }
    }
...
class set : public Tree<Key, Value, Compare>
{
public:
    // include requirments
...
public:
    ...
    public:
        ...
        iterator1& operator++()
        {
            ...
            {
                Key key = node->KeyValue->first;
                node = node->parent;
                while (node != nullptr && key_compare().operator()(node->KeyValue->first, key))
                {
                    node = node->parent;
                }
            }
        ...
        iterator1& operator--()
        {
            ...
            else
            {
                Key key = node->KeyValue->first;
                node = node->parent;
                while (node != nullptr && key_compare().operator()(key, node->KeyValue->first))
                {
                    node = node->parent;
                }
            }
        ...
        iterator1 operator--(int)
        {
            ...
            else
            {
                Key key = node->KeyValue->first;
                node = node->parent;
                while (key_compare().operator()(key, node->KeyValue->first) && node != nullptr)
                {
                    node = node->parent;
                }
            }
    ...
    struct iterator2    // based on list, according to state(condition) - in my case(1 list with iterator that helps us to find necessary information)
    {
    protected:
    ...
    public:
        ...
        value_type& operator*() { return *(*it)->KeyValue; }
    ...
    size_t count(const Key& key)//return number of elements that we choose
    {
    ...
    iterator2 list_begin()//return list begin
    {
        return Tree::listOfNodes.begin();
    }

    iterator2 list_end()//return list end
    {
