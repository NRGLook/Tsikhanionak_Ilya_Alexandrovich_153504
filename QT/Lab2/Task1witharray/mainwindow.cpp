    ...
    , ui(new Ui::MainWindow)
{
    ...
    //проверка - только нужна ли, если пользователь - адекват?
/*    ui->lineEdit_nameAdd->setValidator(new QRegExpValidator(QRegExp("^[a-z]{10}")));//имя
    ui->lineEdit_surnameAdd->setValidator(new QRegExpValidator(QRegExp("^[a-z]{15}")));//фамилия
    ui->lineEdit_patronymicAdd->setValidator(new QRegExpValidator(QRegExp("^[a-z]{18}")));//отчество
    ui->lineEdit_departmentNumAdd->setValidator(new QIntValidator(0,999,this));
    ui->lineEdit_departmentNumAdd->setMaxLength(3);//номер департамента
    ui->lineEdit_posAtWorkAdd->setValidator(new QRegExpValidator(QRegExp("^[a-z]{12}")));//позиция на работе
    ui->lineEdit->setValidator(new QRegExpValidator(QRegExp("^[a-z]{15}")));//фамилия
    ui->lineEdit_2->setValidator(new QIntValidator(0,999,this));
    ui->lineEdit_2->setMaxLength(3);//номер
    ui->lineEdit_3->setValidator(new QIntValidator(0,999,this));
    ui->lineEdit_3->setMaxLength(10);//experience*/
    ui->lineEdit_4->setMaxLength(100);//что-то
    ui->tableWidget->setSelectionMode(QAbstractItemView::NoSelection);
    ui->tableWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->tableWidget->resizeRowsToContents();
    ui->tableWidget->resizeColumnsToContents();
    ui->tableWidget->setHorizontalHeaderLabels(QStringList() << "AverageScore"  << "Name" << "Surname" << "Patronymic" << "AddressofAbiturient" << "DateRegistration");
...
bool MainWindow::test(QString& line, bool mode)
{
    ...
    for (; i < line.size(); i++)    // DEPARTMENTNUMBER
    {
        ...
        {
            QMessageBox::warning(this, "Warning", "Incorrect AverageScore.");
            return false;
        }
    }
    count = 0;
    for (; i < line.size(); i++)    // POSATWORK
    {
        ...
        {
            QMessageBox::warning(this, "Warning", "Incorrect AddressofAbiturient. Must be only letters.");
            return false;
        }
        count++;
    }
...
bool MainWindow::loadFromFile()
{
    ...
    if (listOfWorkers->getCount() > 0)
    {
        ...
        ui->comboBox_5->addItem("AverageScore:");
        ui->comboBox_5->addItem("AddressofAbiturient:");
...
void MainWindow::textOutput()
{
    ...
    ui->tableWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->tableWidget->resizeRowsToContents();
    ui->tableWidget->setHorizontalHeaderLabels(QStringList() << "Name" << "Surname" << "Patronymic" << "AverageScore" << "AddressofAbiturient" << "DateStart");
    ui->tableWidget->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
...
{
    filename = QFileDialog::getOpenFileName(this, "Open a File", "D:/C++/QT/two semester/Lab 2/Task1zaebok/witharray","*txt");
    if(filename.isEmpty())
    {
    ...
    if (loadFromFile()) // 'if' so as not to delete data from incorrect file
    {
        ...
        {
            QMessageBox::warning(this, "Warning","File is overloaded. Max 30 abiturients. Change data and try again.");
            ui->tableWidget->clear();
            listOfWorkers->delAll();
            textOutput();
            checkFile = false;
...
void MainWindow::on_add_Button_clicked()
{
    ...
    if (listOfWorkers->getCount() == 30)
    {
        QMessageBox::warning(this, "Warning","File is overloaded. Max 30 abiturient.");
        return;
    }
    QString line;
    ...
    {
        QMessageBox::warning(this, "Warning","Too big AddressofAbiturient.");
        return;
    }
    ...
    else
    {
        QMessageBox::warning(this, "Warning","Smth incorrect in your input. Name, surname, patronymic: 1-st capital letter and at least 2 letters; "
                                             "AverageScore: only digits (max 3); AddressofAbiturient: only letters, at least 1 letter;");
        return;
    }
    ui->comboBox->clear();
    ui->comboBox_2->clear();
...
void MainWindow::on_searchDepartment_Button_clicked()
{
    ...
    if (line.size() == 0)
    {
        QMessageBox::warning(this, "Warning","Incorrect averagescore: Must be at least 1 digit(max 3).");
        return;
    }
    int found = 0;
    for (int i = 0; i < listOfWorkers->getCount(); i++)
    ...
    {
        QMessageBox::warning(this, "Warning","No such average score in the list.");
        return;
    }
    ui->tableWidget->setHorizontalHeaderLabels(QStringList() << "Name" << "Surname" << "Patronymic" << "AverageScore" << "AddressofAbiturient" << "DateStart");
    ui->tableWidget->setRowCount(found);
...
void MainWindow::on_searchWorkExp_Button_clicked()
{
    ...
    {
        QMessageBox::warning(this, "Warning","No such abiturient in the list.");
        return;
    }
    ui->tableWidget->setHorizontalHeaderLabels(QStringList() << "Name" << "Surname" << "Patronymic" << "AverageScore" << "Addressofabiturient" << "DateStart");
    ui->tableWidget->setRowCount(found);
...
void MainWindow::on_change_Button_clicked()
{
    ...
    else if (currTextParam == "AverageScore:")
    {
        if (line.size() == 0)
        {
            QMessageBox::warning(this, "Warning","Incorrect AverageScore.");
    ...
    else if (currTextParam == "AddressofAbiturient:")
    {
        if (line.size() == 0)
        {
            QMessageBox::warning(this, "Warning","Incorrect AddressAbiturient. Must be at least 1 letter.");
            ui->lineEdit_4->clear();
            return;
        }
        if (line.size() > 30)
        {
            QMessageBox::warning(this, "Warning","Incorrect AddressofAbiturient. Too big tariff posAtWork.");
            ui->lineEdit_4->clear();
            return;
        }
        for (int i = 0; i < line.size(); i++)
            ...
            {
                QMessageBox::warning(this, "Warning","Incorrect addressOfAbiturient. Must be only letters without spaces anywhere.");
                ui->lineEdit_4->clear();
                return;
            }
        // change
...
void MainWindow::on_comboBox_5_currentTextChanged(const QString &currText)
{
    if (checkFile && listOfWorkers->getCount() != 0)
    {
        ...
        else if (currText == "AddressofAbiturient:")
        {
            ui->lineEdit_4->setInputMask("");
