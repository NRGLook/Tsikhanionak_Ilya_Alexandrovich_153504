...
// Добавить информацию о годе
        ...
        ui->label_5->setText("Введенная дата - не правильна. Пожалуйста, попробуйте еще раз!");
    }
...
void MainWindow::createTable(QFile &file){
    ...
    while(!stream.atEnd()){
        ...
        if(!inputIsCorrect(str)){
            ui->label_3->setText("Файл был испорчен!");
            delete[] listOfDates;
            return;
        }
        auto parts = str.split(".");
...
QString MainWindow::getWay(){
    ...
    QFile file(way);
    if(way != "D:/C++/QT/two semester/Lab 1/Task3"){
    if(!file.exists()){
        ui->label_3->setText("Мы не можем использовать файл!");
        return "";
    }
