    ...
    , ui(new Ui::MainWindow)
{
    ...
    if (ui->radioButton->isChecked()) {
        ...
        scn->addItem(sh);
        sh->m_angle = 0;
        ui->sLabel->setText("Square: "+QString::number(sh->m_S));
        ui->pLabel->setText("Perimeter: "+QString::number(sh->m_P));
    ...
    InputWidth = new QLabel("Width:", rectDlg); InputWidth->adjustSize();
    InputHeight = new QLabel("Height:", rectDlg); InputHeight->adjustSize();
    ...
    InputR = new QLabel("Radius: ", circleDlg); InputR->adjustSize();
    LE_R = new QLineEdit(circleDlg);

    squareDlg = new QDialog(this);
    InputA = new QLabel("Side: ",squareDlg); InputA->adjustSize();
    LE_A = new QLineEdit(squareDlg);

    rhombDlg = new QDialog(this);
    Inputr = new QLabel("Radius: ",rhombDlg); InputA->adjustSize();
...
void MainWindow::on_btnRect_clicked()
{
    if (currentFigure == "Rectangle: ") {
        ...
        rectDlg->resize(300, 300);

        InputWidth->move(100, 100);
        InputHeight->move(100, 150);
        LE_Width->resize(60, 30);
        LE_Height->resize(60, 30);
        LE_Width->move(170, 100);
        LE_Height->move(170, 140);

        Enter = new QPushButton("Enter: ", rectDlg); Enter->adjustSize();
        Enter->move(150, 200);
        ...
        InputX3->move(40, 190);
        InputY1->move(180, 50);
        InputY2->move(180, 120);
        InputY3->move(180, 190);
        ...
        Enter = new QPushButton("Enter: ", triangDlg); Enter->adjustSize();
        Enter->move(180, 250);
        ...
        Enter = new QPushButton("Enter: ", circleDlg); Enter->adjustSize();
        Enter->move(150, 200);
        ...
        Enter = new QPushButton("Enter", squareDlg); Enter->adjustSize();
        Enter->move(150, 200);
        ...
        Enter = new QPushButton("Enter: ", rhombDlg); Enter->adjustSize();
        Enter->move(150, 200);
        ...
        Enter = new QPushButton("Enter: ", ovalDlg); Enter->adjustSize();
        Enter->move(150, 200);
        ...
        Enter = new QPushButton("Enter: ", starDlg); Enter->adjustSize();
        Enter->move(150, 300);
        ...
        Enter = new QPushButton("Enter: ", hexDlg); Enter->adjustSize();
        Enter->move(150, 200);
...
void MainWindow::createRect()
{
   ...
   if (!matchHeight.hasMatch() || !matchWidth.hasMatch())
       QMessageBox::critical(rectDlg, "Error!", "Incorrect input! Try again!");
   else {
       rectDlg->close();
...
void MainWindow::createTriang()
{
    ...
    if (!matchX1.hasMatch() || !matchX2.hasMatch() || !matchX3.hasMatch() ||
        ...
        QMessageBox::critical(triangDlg, "Error!", "Incorrect input! Try again!");
    else {
        triangDlg->close();
...
void MainWindow::createCircle()
{
    ...
    if (!matchR.hasMatch())
        QMessageBox::critical(circleDlg, "Error!", "Incorrect input! Try again!");
    else {
        circleDlg->close();
...
void MainWindow::createSquare()
{
    ...
    if (!matchA.hasMatch())
        QMessageBox::critical(squareDlg, "Error!", "Incorrect input! Try again!");
    else {
        squareDlg->close();
...
void MainWindow::createRhomb()
{
    ...
    if (!matchr.hasMatch())
        QMessageBox::critical(rhombDlg, "Error!", "Incorrect input! Try again!");
    else {
        rhombDlg->close();
...
void MainWindow::createOval()
{
    ...
    if (!matchOvalR.hasMatch() || !matchOvalr.hasMatch())
        QMessageBox::critical(ovalDlg, "Error!", "Incorrect input! Try again!");
    else {
        ovalDlg->close();
...
void MainWindow::createStar()
{
    ...
    if (!matchStarR.hasMatch() || !matchStarr.hasMatch() || !matchStarTier.hasMatch()) {
        ...
        QMessageBox::critical(starDlg, "Error!", "Incorrect input! Try again!");
    }
    else {
        starDlg->close();
...
void MainWindow::createHex()
{
    ...
    if (!matchHexR.hasMatch())
        QMessageBox::critical(hexDlg, "Error!", "Incorrect input! Try again!");
    else {
        hexDlg->close();
...
void MainWindow::on_horizontalSlider_valueChanged(int value)
{
    ...
    scn->update();
    ui->sLabel->setText("Square: "+QString::number(sh->m_S * value / 100));
    ui->pLabel->setText("Perimeter: "+QString::number(sh->m_P * value / 100));
