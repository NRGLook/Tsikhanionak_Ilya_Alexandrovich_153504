...
/*Задание 5:Во многих прикладных задачах, таких как поиск в сети или расшифровка 
...
латинского алфавита. Необходимо определить число строк, удовлетворяющих этим 
...
из них содержит два целых числа: L и m (1 ≤ L ≤ 109, 1 ≤ m ≤ 104). Вторая и 
...
bool ass1(char* arr1, char* arr2, int length, int difference) {
	int n = 0;
...
bool ass2(char* arr1, char* arr2, int length, int difference) {
	int N = 1;
	...
	while (T <= t) {
		...
		if (L1 > L || L2 > L) NUMBER = 0;
			...
			if (L >= L1 + L2) NUMBER = 2 * pow(26, (L - (L1 + L2)));
				...
				int diff = L1 + L2 - L;
				bool alpha = ass1(s1, s2, L1, diff);
				bool betha = ass2(s2, s1, L2, diff);
		...
		cout << "\n\nThe ammount of possible cases according to youe data is equal to   " << NUMBER % m << "\n\n\n";
		T++;
	}
